...
| `docs/testing-map.md` | Contributor guide | The mechanics behind `AGENTS.md`'s rule for where a test goes: the eight test homes, what runs each, which gate a merge and which do not, and the three traps — a directory no `PYTHON_TEST_DIRS` glob reaches, a `bench/tasks/` case with no `domain:` slug, and registering a case in `TASKS` before its activation blockers clear. | Test-tier routing, `make test-python`/`test-bench`, `optional: true` Prow job, domain coverage accounting, `VerificationCorrectness` floor | AI coding agents and human contributors; mechanics only, `AGENTS.md` owns the rule |
