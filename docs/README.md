...
| `bench/tasks/incident-triage/README.md` | Component README | The incident-triage task group: what each of the fourteen report-contract tasks covers, how to run one, which four run in the PR evaluation, and the two authoring traps (one-line checklist bullets; assert substance, not vocabulary). | Task group, judge criteria, CI subset | Developers editing or adding incident-triage eval tasks |
